            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    @if (empty($data_output_all_product))
                        <div class="container">
                            <div class="row">
                                <h3 class="d-flex justify-content-center" style="color: #00000">No Data Found For Services
                                </h3>
                            </div>
                        </div>
                    @else
                        @foreach ($data_output_all_product as $product)
                            <div class="col-lg-4 col-md-4 col-sm-4">
                                <div class="card">
                                    <img src="{{ Config::get('DocumentConstant.PRODUCT_VIEW') }}{{ $product['image'] }}"
                                        alt="{{ strip_tags($product['title']) }}" class="img-fluid" />
                                        ...
                                        <div class="col-md-3">
                                            <nav class="article_nav">
                                                <div class="nav nav-tabs article_tab" id="nav-tab" role="tablist">
                                                    @forelse($all_services as $key=>$categories_data)
                                                        <button
                                                            class="nav-link article_tab_link {{ $loop->first ? 'active' : '' }}"
                                                            id="id_{{ $categories_data->id }}" data-bs-toggle="tab"
                                                            data-bs-target="#data_id_{{ $categories_data->id }}"
                                                            onclick="getServices('{{ $categories_data->id }}')"
                                                            type="button" role="tab" aria-controls="nav-profile"
                                                            aria-selected="{{ $loop->first ? 'true' : 'false' }}">{{ $categories_data->product_name }}</button>
                                                            @empty
                                                            <div class="alert alert-primary" role="alert">
                                                                No Data Found
                                                            </div>
...
$(document).ready(function() {
    // Show data for the first category if available
    var firstCategoryId = '{{ $all_services->isNotEmpty() ? $all_services->first()->id : '' }}';
    if (firstCategoryId) {
